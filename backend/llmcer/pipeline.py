...
from llmcer.event_logger import emit, set_context
...
def full_process_id_list(block_idx, id_list, vectors, simi_matrix, df, the_threshold,
                         max_k=5, chunk_size=10, retry_attempts=2):
    set_context(f"block_{block_idx:03d}")
    print(f"  [Block {block_idx}] Processing block with {len(id_list)} items...")
    ...
    # 2. Clustering
    n_clusters = elbow_method(vectorized_data, max_k=max_k)
    labels = kmeans_clustering(vectorized_data, n_clusters)
    clusters_labels = format_output(id_list, labels)
    emit("kmeans_result", "separation", f"Block K-Means: k={n_clusters}, {len(id_list)} records", {
    ...
    emit("classify_result", "separation", f"LLM classify done: {len(classified_results)} groups", {
    ...
    for i, row_slice in enumerate(target_list):
        ...
        array_new, api_call, use_t, use_tok, in_tok, out_tok, mdg_count = llm_seperate(row_slice, df, simi_matrix, the_threshold, chunk_size=chunk_size, retry_attempts=retry_attempts)
    ...
    emit("separate_result", "separation", f"Block separation done: {len(find_back_matrix)} clusters", {
        "clusters": find_back_matrix,
    })
...
def seperate_parallel(vectors, simi_matrix, merge_clusters_pre, df, the_threshold,
                      max_workers=5, max_k=5, chunk_size=10, retry_attempts=2):
    sperate_result = []
    api_call_time_all = 0
    sperate_time = 0
    sperate_token = 0
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = [executor.submit(full_process_id_list, idx, id_list, vectors, simi_matrix, df, the_threshold,
                                   max_k=max_k, chunk_size=chunk_size, retry_attempts=retry_attempts) for idx, id_list in enumerate(merge_clusters_pre)]
