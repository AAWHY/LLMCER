...
def lsh_block(vectors, data, similarity_threshold, hash_size=None, input_dim=None, num_hashtables=None):
    hash_size = hash_size or LSH_HASH_SIZE
    input_dim = input_dim or LSH_INPUT_DIM
    num_hashtables = num_hashtables or LSH_NUM_HASHTABLES
    ...
    emit("block_result", "blocking", f"LSH blocking done, {len(clusters_array)} blocks", {
        "num_blocks": len(clusters_array),
        "block_sizes": [len(b) for b in clusters_array],
        "blocks": clusters_array,
    })
    print("lsh done")
        ...
        for r_j in current_cluster:
            ...
            if max_inter_sim > -1.0:
                if avg_intra_sim < max_inter_sim:
                    emit("mdg_fail", "separation", f"MDG misclustering: record {r_j} intra_sim={avg_intra_sim:.4f} < inter_sim={max_inter_sim:.4f}", {
                        "record": r_j,
                        "cluster": list(current_cluster),
                        "intra_sim": float(avg_intra_sim),
                        "inter_sim": float(max_inter_sim),
                    })
                    return False
